...
class LanguageController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $language->is_active =  ($request->is_active == 'on') ? 1 :0;
        $language->rtl = ($request->rtl == 'on') ? 1 :0 ;
    ...
    public function keywordUpdate(Request $request, $id)
    {
        ...
        if ($content === 'null') {
            return back()->with('danger', 'At Least One Field Should Be Fill-up');
        }
    ...
    public function storeKey(Request $request, $id)
    {
        ...
        if (array_key_exists($requestKey, json_decode($items, true))) {
            return back()->with('danger', "`$requestKey` Already Exist");
        } else {
