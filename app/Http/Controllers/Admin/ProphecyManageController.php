...
class ProphecyManageController extends Controller
{
    ...
    public function saveMatch(Request $request)
    {
        ...
        $in = request()->except('_token', 'start_time', 'end_time','team1_image','team2_image');
        $start_date = $request->start_date . ' ' . $request->start_time;
        $end_date = $request->end_date . ' ' . $request->end_time;

        $in['slug'] = str_slug($request->team1.' Vs '.$request->team2);
        ...
        if ($request->hasFile('team1_image')) {
            try {
                $team1_image = $this->uploadImage($request->team1_image, config('location.team.path'), config('location.team.size'));
            } catch (\Exception $exp) {
                return back()->with('danger', 'Image could not be uploaded.');
            }
        }
        $in['team1_image'] = $team1_image;
        ...
        if ($request->hasFile('team2_image')) {
            try {
                $team2_image = $this->uploadImage($request->team2_image, config('location.team.path'), config('location.team.size'));
            } catch (\Exception $exp) {
                return back()->with('danger', 'Image could not be uploaded.');
            }
        }
        $in['team2_image'] = $team2_image;
    ...
    public function updateMatch(Request $request)
    {
        ...
        if ($request->hasFile('team1_image')) {
            try {
                $old1 = $data->team1_image ?: null;
                $team1_image = $this->uploadImage($request->team1_image, config('location.team.path'), config('location.team.size'),$old1);
            } catch (\Exception $exp) {
                return back()->with('danger', 'Image could not be uploaded.');
            }
        }
        $in['team1_image'] =  $team1_image ?? $data->team1_image;



        if ($request->hasFile('team2_image')) {
            try {
                $old2 = $data->team2_image ?: null;
                $team2_image = $this->uploadImage($request->team2_image, config('location.team.path'), config('location.team.size'),$old2);
            } catch (\Exception $exp) {
                return back()->with('danger', 'Image could not be uploaded.');
            }
        }
        $in['team2_image'] =  $team2_image ?? $data->team2_image;;
    ...
    public function makeWinner(Request $request)
    {
        ...
        foreach ($winner as $dd) {
            ...
            Trx::create([
                ...
                'type' => '*',
                'title' => "<strong>Event:</strong> " . optional($dd->match)->name." <br>( <strong> Ques:</strong> ".optional($dd->ques)->question. " <strong>, Threat: ". optional($dd->betoption)->option_name ." => Win)</strong>",
                'trx' => getTrx(),
            ]);
        }
