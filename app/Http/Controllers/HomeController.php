...
class HomeController extends Controller
{
    ...
    public function sendEmailCode(Request $request)
    {
        ...
        if (Carbon::parse($user->email_time)->addMinutes(2) > Carbon::now()) {
            ...
            session()->flash('danger', trans('You can resend Verification Code after').  " $delay " . trans('minutes'));
        } else {
    ...
    public function submitPassword(Request $request)
    {
        ...
        try {
            ...
            session()->flash('danger', trans($e->getMessage()));
            return back();
        }
    }
    ...
    public function moneyTransferConfirm(Request $request)
    {
        ...
        if ($request->amount < $basic->min_transfer) {
            session()->flash('danger', trans('Minimum Transfer Amount') . " $basic->min_transfer " . trans($basic->currency));
            return back()->withInput();
        }
        if ($request->amount > $basic->max_transfer) {
            session()->flash('danger', trans('Maximum Transfer Amount') . " $basic->max_transfer " . trans($basic->currency));
            return back()->withInput();
        }
        ...
        if ($user->balance >= ($request->amount + $transferCharge)) {
            ...
            if (Hash::check($request->password, $user->password)) {
                ...
                if (isset($sendMoneyCheck) && Carbon::parse($sendMoneyCheck->send_at) > Carbon::now()) {
                    ...
                    session()->flash('danger', trans('You can send money to this user after  delay') . " $delay " . trans('minutes'));
                    return back()->withInput();
                } else {
            ...
            if ($user->balance >= $request->invest_amount) {
                ...
                if ($predictOption->min_amo <= $request->invest_amount) {
                    ...
                    if ($lastPredictionIn && Carbon::parse($lastPredictionIn->created_at)->addSeconds(15) > Carbon::now()) {
                        ...
                        $delay = $time->diffInSeconds(Carbon::now());
                        $delay = gmdate('i:s', $delay);
                        session()->flash('danger', trans('You can next predict after') . " $delay " . trans('seconds in') .' ' . trans($predictOption->option_name));
                        return back();
                    }

                    if (($predictIn + $request->invest_amount) > $predictOption->bet_limit) {
                        session()->flash('danger',trans('Your Prediction limit over in'). "( ".trans($predictOption->option_name).")");
                        return back();
                    }
                    ...
                    Trx::create([
                        ...
                        'charge' => 0,
                        'type' => '-',
                        'title' => 'Predict in ' . $mm->team1 .' Vs '.$mm->team2 . "<strong> ( " . $trxQ . "  =>   $predictOption->option_name )</strong>",
                        'trx' => $tr
                    ]);

                    session()->flash('success', trans('Successfully Prediction in'). " $predictOption->option_name");
                    return back()->withInput();
                } else {
                    session()->flash('danger', trans("Minimum Amount"). " $predictOption->min_amo " . trans($basic->currency));
                    return back()->withInput();
                }
            } else {
                session()->flash('danger', trans('Insufficient Balance'));
                return back()->withInput();
            }
        } else {
            session()->flash('danger', trans("Time has been expired"));
            return back()->withInput();
        }
    }
    ...
    public function withdrawMoneyRequest(Request $request)
    {
        ...
        if ($request->amount < $method->withdraw_min) {
            session()->flash('danger', trans('Minimum Withdraw Amount') . " ".round($method->withdraw_min, 2)." " .  trans($basic->currency));
            return back();
        }
        if ($request->amount > $method->withdraw_max) {
            session()->flash('danger', trans('Maximum Withdraw Amount') . " ". round($method->withdraw_max, 2 ). " ". trans($basic->currency));
            return back();
        }
    ...
    public function withdrawReqSubmit(Request $request)
    {
        ...
        if (formatter_money($withdraw->net_amount) > $user->balance) {
            ...
            session()->flash('success',trans('Withdraw request Successfully Submitted. Wait For Confirmation.'));
            return redirect()->route('user.withdraw-money');
