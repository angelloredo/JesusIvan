...
class PaymentController extends Controller
{
    ...
    public function paymentRequest(Request $request){
        ...
        if (!$gate) {
            return response(['errors'=>[trans('Invalid Payment Method')]]);
        }


        if ($gate->min_amount > $request->amount || $gate->max_amount < $request->amount) {
            return response(['errors'=>[trans('Please Follow Deposit Limit')]]);
        }
        ...
        if($data->gateway_currency()->crypto == 1){
            $crypto = 1;
            $conversion_rate = 1 . ' '.$general->currency_sym . ' = '. formatter_money($data->rate,8)  . ' USD';
            $in = 'In USD : '.formatter_money($data->final_amo + 0);
        }else{
            $crypto = 0;
            $conversion_rate = 1 . ' '.$general->currency_sym . ' = '. formatter_money($data->rate)  . ' '.$data->method_currency;
            $in = "In $data->method_currency : ".formatter_money($data->final_amo + 0);
        }
        ...
        $result['logs'] = [
            ...
            'amount' => trans('Amount') . ' '. formatter_money($data->amount) . ' '. $general->currency_sym,
            'charge' => trans('Charge') . ' '. formatter_money($data->charge). ' '. $general->currency ,
            'payable' => trans('Payable') . ' '. formatter_money($data->final_amo). ' '. trans($data->method_currency) ,
            'method_code' => $data->method_code,
            'conversion_rate' => $conversion_rate,
            'in' =>$in,
            'crypto' => $crypto,
            'crypto_msg' => 'Conversion with: '.$data->method_currency . ' and final value will Show on next step'
        ];
    ...
    public function payNow()
    {
        ...
        if (is_null($deposit)) {
            session()->flash('danger',trans('Invalid Payment Request'));
        ...
        if ($deposit->method_code >= 1000) {
            ...
            session()->flash('danger',trans('Your deposit request is queued for approval.'));
            return redirect()->route('payment');
        }
                        ...
                        if ($inVal->type == 'file') {
                            if ($request->hasFile($inKey)) {
                                try {
                                    ...
                                    session()->flash('danger', trans('Could not upload your') . ' '.  $inKey);
